...
**IMPORTANT FINDING for `manasight/manasight-parser#232`:** the submitted deck ("Boros Tokens") is **Standard-registered**, so the EventSetDeckV3 `Format` attribute reads **`Standard`**, NOT the queue/match format. The deck `Format` attribute = the deck's registered/build format (narrowest legal), not the match format; for format-named queues the precise format is carried by `EventName` (`Explorer_Play`), and the deck `Format` can under-report. Contrast with `2026-06-17_1543_alchemy`, where the Alchemy-registered deck's `Format` happened to match the queue.
...
| Deck / archetype | Mardu Energy Timeless (Lurrus companion; Timeless-only cards) |
| Record | 1 game played (result not recorded) |
...
| Deck / archetype | Hei Bai, Forest Guardian (100-card singleton commander) |
| Record | 1 game played (result not recorded) |
...
| Deck / archetype | Boros Tokens (Standard-registered) |
| Record | 1 game played (result not recorded) |
